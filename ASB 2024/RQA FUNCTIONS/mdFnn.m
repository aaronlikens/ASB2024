function [fnnPerc, embTimes, dim] = mdFnn(data, tau, varargin)
...
if (doPlot)  % plot results
    figure()
    plot(fnnPerc,'--o', 'LineWidth', 2, 'MarkerSize', 4, 'Color', '#097BAB',...
        'MarkerEdgeColor', '#097BAB')
...
for i = 2:length(embTimes)-1
        % Looks for 0 percentage or minimum.
        if (fnnPerc(i)<=0)||((fnnPerc(i-1)>fnnPerc(i)&&(fnnPerc(i)< fnnPerc(i+1))))
            dim= i;
            break
        end
        % Looks for a change in %FNN below a threshold.
        if fnnPerc(i-1)-fnnPerc(i)<=0.001
            dim = i-1;
            break
        end
