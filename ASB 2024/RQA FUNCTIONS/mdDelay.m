...
%
%     minAMI: Mark the time lag at which first local minimums are attained
%     0 or 1. Default = 0
%
%   Outputs:
...
% Optional parameter: minAMI
defaultminAMI = 0;
checkminAMI = @(x) validateattributes(x, {'logical'}, {'nonempty'});
...
addOptional(parser, 'minAMI', defaultminAMI, checkminAMI);
...
switch plotType
    ...
    case 'all'
        plotAllMI(auto_mi, threshold, lags, minAMI);
    case 'both'
        plotMeanMI(auto_mi, threshold);
        plotAllMI(auto_mi, threshold, lags, minAMI);
    case 'none'
...
function plotAllMI(ami, threshold, lags, minAMI)
[nlag, ncol] = size(ami);
maxlag = nlag - 1;
figure();
hold off
for c = 1:ncol
    plot(0:maxlag, ami(:, c), 'b', 'LineWidth', 2, 'Color', '#097BAB');
    hold on
    if minAMI == 1
        
        plot(lags(c), ami(lags(c)+1,c), 'o', 'MarkerFaceColor', '#D71920', 'MarkerEdgeColor', '#D71920', 'MarkerSize', 8);
        plot([lags(c) lags(c)],[0 ami(lags(c)+1,c)], '--', 'Color', '#D71920', 'LineWidth', 2)
    end
end
%     refline(0, threshold)
limits = ylim;
...
xlabel('Time lag')
ylabel('Mutual Information')
%     text(floor(maxlag/2),2,['tau = ' sprintf('%.0f,' , lags) ', mean = ' string(mean(lags)) '.'])
end